...
public class Car : MonoBehaviour {
    ...
    public GameObject sideCarLeft, sideCarRight;
    ...
    private void Update() {
        if (onVisibleInCamera) {
            ...
            if (mainNumOfLane != numOfLane) {
                ...
                RaycastHit[] hits = Physics.RaycastAll(transform.position, dir, 0.25f);
                for (int i = 0; i < hits.Length; ++i) {
                    RaycastHit hit = hits[i];
                    if (hit.collider.gameObject.name == "SideCar" && hit.collider.gameObject != sideCarLeft && hit.collider.gameObject != sideCarRight) {
                        p = false;
                        break;
                    }
                }
                ...
                canChangeLane = p;
                if (!p) speed = Math.Max(speed - braking, 0f);
            }
        }
