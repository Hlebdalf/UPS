    ...
    IEnumerator AsyncGen() {
        for (int i = 0; i < RoadsClass.objects.Count; ++i) {
            ...
            for (int lenIt = 1; lenIt < len; ++lenIt) {
                ...
                float widthHouse1 = (int)BuildsClass.preFubs[BuildsClass.idxParking].GetComponent <BoxCollider> ().size.x * BuildsClass.preFubs[BuildsClass.idxParking].transform.localScale.x;
                float lenHouse1 = (int)BuildsClass.preFubs[BuildsClass.idxParking].GetComponent <BoxCollider> ().size.z * BuildsClass.preFubs[BuildsClass.idxParking].transform.localScale.x;
        ...
        while (BuildsClass.parkings.Count < GenerationClass.averageCntParkingInDistrict * 4) {
            ...
            if (FieldClass.objects[(int)minBlock.point.x + FieldClass.fieldSizeHalf, (int)minBlock.point.z + FieldClass.fieldSizeHalf] == null) {
                ...
                float widthHouse = (int)BuildsClass.preFubs[BuildsClass.idxParking].GetComponent <BoxCollider> ().size.x * BuildsClass.preFubs[BuildsClass.idxParking].transform.localScale.x;
                float lenHouse = (int)BuildsClass.preFubs[BuildsClass.idxParking].GetComponent <BoxCollider> ().size.z * BuildsClass.preFubs[BuildsClass.idxParking].transform.localScale.x;
